...
The guide documents local setup, server-side credentials, per-call authorization, E.164 allowlisting, duplicate prevention, cancellation limits, and the boundaries of the demo. Public testing requires no credentials or telephone calls. After an initial voicemail attempt, a completed consenting role-play test returned structured facts. All seven exported recipient fields match the supplied UI screenshot, including unknown availability. The updated video is a labeled captured-result replay; the public preview remains fictional. See docs/LIVE-RESULT-REVIEW.md in the linked implementation.
