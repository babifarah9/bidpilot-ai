...
import { result } from '../tests/live-result-replay.mjs';
...
await p.evaluate(()=>{const x=document.createElement('div');x.textContent='RECORDED LIVE RESULT REPLAY · No new call · Synthetic narration';x.style.cssText='position:fixed;top:0;left:0;right:0;z-index:99999;pointer-events:none;background:#fbbf24;color:#0f172a;padding:10px;text-align:center;font:bold 15px sans-serif';document.body.append(x);});
...
await panel.getByLabel('Facts CALL-E may verify').fill('This is a consenting supplier role-play demonstration. Identify yourself as AI. Ask about availability, lead time, service coverage, certification status, voluntary non-binding pricing, and required human follow-up. Do not discuss RFP compliance or make commitments.');
...
await panel.getByText(result.recipients[0].structured_result.notes,{exact:true}).waitFor();
...
await panel.getByRole('button',{name:'Download verification evidence'}).click();
const download=await downloadPromise;
const exported=JSON.parse(fs.readFileSync(await download.path(),'utf8'));
assert.deepEqual(exported.recipient_structured_result,result.recipients[0].structured_result);
assert.deepEqual(exported.overall_structured_result,result.structured_result);
assert.equal(exported.callId,result.id);
assert.ok(!JSON.stringify(exported).includes('fixture-operator-token-not-real'));
console.log('Scene 4: exact recipient facts rendered');await pause(25000);
await p.evaluate(()=>{const x=document.createElement('div');x.style.cssText='position:fixed;inset:0;z-index:99998;background:#071b25;color:white;display:flex;align-items:center;justify-content:center;padding:100px;font:30px sans-serif';x.innerHTML='<div><p style="color:#5eead4;font-size:18px">BIDPILOT VOICE</p><h1>Reviewable supplier evidence.<br>Human decisions.</h1><p>✓ TypeScript and production build passed</p><p>✓ Backend safeguards and simulated result checks passed</p><p style="color:#fbbf24">Completed role-play test: all 7 API fields match the screenshot<br>Availability remains unknown; human follow-up required</p></div>';document.body.append(x);});
console.log('Scene 5: live response mapping passed and remaining limits');await pause(26000);
